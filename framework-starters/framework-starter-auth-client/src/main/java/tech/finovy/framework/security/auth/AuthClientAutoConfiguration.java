package tech.finovy.framework.security.auth;
...
import tech.finovy.framework.security.auth.core.account.CustomDaoAuthenticationProvider;
import tech.finovy.framework.security.auth.core.account.CustomPasswordEncoderEnhance;
import tech.finovy.framework.security.auth.core.account.UserDetailsRepository;
import tech.finovy.framework.security.auth.core.config.AuthorizationExtensionProperties;
import tech.finovy.framework.security.auth.core.exception.SimpleAccessDeniedHandler;
import tech.finovy.framework.security.auth.core.exception.SimpleAuthenticationEntryPoint;
import tech.finovy.framework.security.auth.core.filter.CodeModeAuthenticationFilter;
import tech.finovy.framework.security.auth.core.filter.JwtAuthenticationFilter;
import tech.finovy.framework.security.auth.core.filter.PreLoginFilter;
import tech.finovy.framework.security.auth.core.filter.TokenAuthenticationFilter;
import tech.finovy.framework.security.auth.core.handler.*;
import tech.finovy.framework.security.auth.core.oidc.AuthorizationProvider;
import tech.finovy.framework.security.auth.core.oidc.CustomOidcUserService;
import tech.finovy.framework.security.auth.core.oidc.DelegateOAuth2AuthorizationCodeGrantRequestEntityConverter;
import tech.finovy.framework.security.auth.core.oidc.DelegateOAuth2UserRequestEntityConverter;
import tech.finovy.framework.security.auth.core.token.jwt.JwtProperties;
...
import tech.finovy.framework.security.auth.extend.CustomPasswordEncoder;
import tech.finovy.framework.security.auth.extend.UserDetailService;
import tech.finovy.framework.security.auth.extend.UsernameAndPasswordService;
import tech.finovy.framework.security.auth.service.ClientProviderHolder;
import tech.finovy.framework.security.auth.service.UserDetailServiceImpl;
...
public class AuthClientAutoConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(ClientRegistrationRepository clientRegistrationRepository
        ...
        http.csrf(csrf -> {
                ...
                .oauth2Login(oauth2Login ->
                        oauth2Login.successHandler(successHandler)
                                ...
                                .loginProcessingUrl(extensionConfig.getLoginProcessingUrl())
        ...
        // extension
        ClientProviderHolder.setAuthorizationExtensionProperties(extensionConfig);
        ClientProviderHolder.setClientRegistrationRepository(clientRegistrationRepository);
    ...
    public DelegateOAuth2UserRequestEntityConverter delegateOAuth2UserRequestEntityConverter() {
        ...
        Arrays.stream(AuthorizationProvider.values()).forEach(oAuth2Provider -> {
    ...
    InMemoryClientRegistrationRepository clientRegistrationRepository(OAuth2ClientProperties properties, AuthorizationExtensionProperties config) {
        Set<String> registrationIds = Arrays.stream(AuthorizationProvider.values())
                .map(AuthorizationProvider::getRegistrationId)
        ...
        List<ClientRegistration> registrations = Arrays.stream(AuthorizationProvider.values())
                .map(oAuth2Provider -> {
                    try {
    ...
    public DelegateOAuth2AuthorizationCodeGrantRequestEntityConverter oAuth2AuthorizationCodeGrantRequestEntityConverter() {
        ...
        Arrays.stream(AuthorizationProvider.values()).forEach(customOAuth2Provider -> {
