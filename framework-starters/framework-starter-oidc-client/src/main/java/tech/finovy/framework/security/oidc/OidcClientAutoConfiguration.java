...
public class OidcClientAutoConfiguration {
    ...
    @Bean
    public CustomAuthenticationSuccessHandler customAuthenticationSuccessHandler(@Nullable AuthorizationCallbackHandler callbackHandler, UserDetailService userDetailService) {
        return new CustomAuthenticationSuccessHandler(callbackHandler, userDetailService);
    }
    ...
    InMemoryClientRegistrationRepository clientRegistrationRepository(OAuth2ClientProperties properties) {
        ...
        List<ClientRegistration> registrations = Arrays.stream(OidcAuthorizationProvider.values())
                .map(oAuth2Provider -> {
                    try {
                        return oAuth2Provider.getBuilder(properties);
                    } catch (Exception e) {
