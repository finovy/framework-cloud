...
@Slf4j
@LoadLevel(name = "TestGlobalDisruptorListener", order = 1, scope = Scope.SINGLETON)
public class TestGlobalDisruptorListener extends AbstractDisruptorListener {
    @Autowired
    private DisruptorEventConfiguration configuration;

    @Override
    public void onEvent(DisruptorEvent event, int handlerId) {
        log.info("GLOBAL---------------handlerId-->{}", handlerId);
    }
